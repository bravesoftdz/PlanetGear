...
uses
  ...
  constants, Planet_Gear_3K, Materials, Vcl.ComCtrls, Vneshn, Vnut, satellit;
...
Var
  Planet: TPlanet_Gear_3K;
  I: Integer;
begin
  Input.pe := strtofloat(Edit5.text);
 // Input.ih := strtofloat(Edit1.Text);
  Input.ih_1:=strtofloat(Edit6.text);
  Input.ih_2:=strtofloat(Edit7.text);
  ...
  if ComboBox13.Text = 'Фрезерование' then
    Input.Wikr_5 := 0;
  if ComboBox13.Text = 'Шлифование' then
    Input.Wikr_5 := 1;
  if ComboBox13.Text = 'Полирование' then
    Input.Wikr_5 := 2;
  ...
  if ComboBox15.Text = 'Поковка' then
    Input.Zagotowka_5 := 0;
  if ComboBox15.Text = 'Штамповка' then
    Input.Zagotowka_5 := 1;
  if ComboBox15.Text = 'Прокат' then
    Input.Zagotowka_5 := 2;
  if ComboBox15.Text = 'Отливка' then
    Input.Zagotowka_5 := 3;
  ...
  if ComboBox16.Text = 'Ra1.6' then
    Input.Ra_5 := 0;
  if ComboBox16.Text = 'Ra3.2' then
    Input.Ra_5 := 1;
  if ComboBox16.Text = 'Ra6.3' then
    Input.Ra_5 := 2;

  Input.Termobr_Material_1 := ComboBox4.Text;
  Input.Termobr_Material_2 := ComboBox6.Text;
  Input.Termobr_Material_3 := ComboBox10.Text;
  Input.Termobr_Material_5 := ComboBox14.Text;
  Input.Lh := strtoint(Edit4.Text);

  Planet := TPlanet_Gear_3K.create(Input);
  Planet.raschet;
  ...
  begin
    ...
    Lines.Add('Марка стали колеса 2 - ' + Output.Material_5);
    ...
    Lines.Add('Термообработка колеса 2 - ' + Output.termobr_5);
    ...
    Lines.Add('Количество зубьев сателлита 2 - ' + Output.z_4.ToString);
    Lines.Add('Количество зубьев колеса 2 - ' + Output.z_5.ToString);
    ...
    Lines.Add('ширина венца сателлита 2 - ' + floattostrf(Output.B_4,ffFixed,7,2)+', mm');
    Lines.Add('ширина венца колеса 2 - ' + floattostrf(Output.B_5,ffFixed,7,2)+', mm');
    ...
    Lines.Add('Диаметр окружности вершин сателлита 2 - ' + floattostrf(Output.Da_4,ffFixed,7,2)+', mm');
    Lines.Add('Диаметр окружности вершин колеса 2 - ' + floattostrf(Output.Da_5,ffFixed,7,2)+', mm');
    ...
    Lines.Add('Делительный диаметр сателлита 2 - ' + floattostrf(Output.D_4,ffFixed,7,2)+', mm');
    Lines.Add('Делительный диаметр колеса 2 - ' + floattostrf(Output.D_5,ffFixed,7,2)+', mm');
    ...
    Lines.Add('Контактное напряжение между сателлитом(2) и колесом(2) - ' +
      floattostrf(Output.Sigma_H_45,ffFixed,7,2)+', МПа');
    ...
    Lines.Add('Напряжения изгиба в зубе сателлита(2) - ' +
      floattostrf(Output.Sigma_F_4,ffFixed,7,2)+', МПа');
...
begin
  DrawKol(Output.m,0,Output.B_1,round(Output.z_1),MyDIR);
 // DrawKol(Output.m,0,Output.B_2,round(Output.z_2),MyDIR);
  DrawKolS(Output.m,0,Output.B_2,Output.m,0,Output.B_4, Input.pe, round(Output.z_2), round(Output.z_4), MyDIR);
  DrawKolVnut(Output.m,0,Output.B_3,round(Output.z_3),MyDIR);
  DrawKolVnut(Output.m,0,Output.B_5,round(Output.z_5),MyDIR);
end;
...
begin
  ...
  ComboBox13.Text := finish[0];
  for I := 0 to Length(finish) - 1 do
  begin
    ComboBox13.Items.Add(finish[I]);
  end;
  ...
  ComboBox15.Text := PatternProcess[0];
  for I := 0 to Length(PatternProcess) - 1 do
  begin
    ComboBox15.Items.Add(PatternProcess[I]);
  end;
  ...
  ComboBox16.Text := roughness[0];
  for I := 0 to Length(roughness) - 1 do
  begin
    ComboBox16.Items.Add(roughness[I]);
  end;
  ...
  ComboBox14.Text := marka_termobr[1];
  for I := 1 to 38 do
  begin
    ComboBox14.Items.Add(marka_termobr[I]);
  end;
